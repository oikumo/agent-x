...
- [x] **R4 (meta_harness_dsl): state hygiene + latent bugs** — DONE (2026-07-26): ① ledger.jsonl rotation — 64 KB hot cap (LEDGER_CAP_BYTES, cross-language pinned), overflow appends into ledger-YYYYMM.jsonl, readers scan latest archive + hot (omt_shared.ts readLedger/appendLedger/rotateLedgerIfNeeded; tdd/state.py mirror; call sites re-pointed: session_state/think_gate/omt_status/omt_think); fixes F21 think-gate per-edit parse latency. ② omt_done reachable — run_full_suite now `-m "not opencode_live" -rf` (F7 120 s-timeout blocker dead) + KNOWN_SUITE_FAILURES allowlist (feature_018 ×3 + window-flaky real-ledger gate probe; F6); dry-run ok in 40 s. ③ TESTLIST tests/ bootstrap documented in META_HARNESS.md (decision b: omt_skip canary; TDD_BOOTSTRAP/TDD_DONE_ALLOWLIST lines). Tests: test_ledger_rotation.py (9: Py rotation contract ×5, bun TS contract ×1, allowlist helpers ×3) + 2 pins (cap cross-language, no raw ledgerPath access); full static 995 passed + 3 known feature_018 ✓; receipt refreshed. Fallout: mid-cycle ImportError broke e2e between rounds → recovered by `git checkout` of state.py/cli.py (clean files are first-editable) + full-file Writes; lesson — keep each guarded file import-consistent per round.
...
- [~] **META HARNESS DSL** (refactor.meta_harness_dsl; supersedes refactor.meta_harness) — executing `.ws/sandbox/meta_harness_refactor_plan.md` (baseline anchor `a7163df`; workstreams R0–R8). **Resume:** R0 ✓ · R6 ✓ (reindex+rewrite class DELETED, tombstone fold, compact digest, S3 fallback kept Tier 1c) · R1 ✓ (shared lib + 4 plugins rewired) · R2 ✓ DONE (2026-07-26, verify-only): enforcer split verified — 7 modules in `.opencode/lib/enforcer/` (session_state/nav_gate/receipt_guard/phase_gate/tdd_hats/think_gate/mvc_after), `omt_enforcer.ts` = thin composition root (single default export, B2), `omt_think.ts` slimmed tools-only (S6: digest machinery → omt_shared.ts, emission → enforcer sessionBootstrap, ONE Set), isOmtHarness += `.opencode/lib/enforcer/`. Verify results: ① pin/e2e batch 18/18 ✓ (import-path fix confirmed) ② full static 984 passed + 3 known feature_018 ✓ ③ B6 live smoke 1/1 ✓ (real opencode 1.18.5; first tool result carried NAVIGATION TIP + 💡 TA: from ONE bootstrap) ④ live-smoke docstring de-staled (line refs → mechanism names) ⑤ omt_complete → Done. Fallout fixed: 3 feature_016 enforcer source-pins retargeted monolith → `lib/enforcer/{tdd_hats,phase_gate}.ts` (canary omt_skip{scope:tests} logged). Deviations: old enforcer:963 TA: re-anchored into think_gate.ts (injectThoughtsOnRead — the branch it describes), not mvc_after as plan guessed. · R3 ✓ DONE (2026-07-26): tdd_check.py 825 lines → `scripts/omt/tdd/` package (state.py ledger/snapshot/runners, ast_checks.py true-RED/coverage, gates.py two-hats/after-edit/validate-exit, cli.py cycle verbs + dispatch); tdd_check.py = compat shim (CLI + full module-API re-exports, zero call-site changes). UNLOCK_WINDOW_MS pin retargeted → tdd/state.py; e2e HARNESS_FILES += 5 package files (canary omt_skip{scope:tests} logged). Verify: test_tdd_check 29/29 ✓ · harness subset 75/75 ✓ · feature_016 22/22 ✓ · full static 984 passed + 3 known feature_018 ✓ (exact R2 baseline) · receipt refreshed. · **R4 ✓ DONE (2026-07-26)** — see the dedicated R4 task entry above; resume at R5 or R7-residue per plan sequencing (R3 ∥ R4 ∥ R5 done → R7 tail)· R5 ☐ · R7 ☐ · R8(HDL) ☐. Live suite REDUCED by directive: 17 tests → 1 minimal smoke. <!-- id:T-HDL prio:high agent:true -->
...
- **META HARNESS DSL** — see task list entry (T-HDL): R0/R1/R2/R3/R6 DONE, **resume at R4** per `.ws/sandbox/meta_harness_refactor_plan.md`.
- feature_001.session_user_objectives_driven_by_Petri_Net — scope & success criteria unset.
- feature_002.rag_retrieval_augmented_generation — scope & success criteria unset.
