...
- [~] **feature_027.rag_v2** — Design phase COMPLETE 2026-08-15; advanced to Programming (TDD auto-activates D7). Shipped `design_001_retrieve_offload_delegate.md` + companion `operation_spec_001_rag_v2_service_and_tools.md` to `4.design/features/feature_027.rag_v2/` (architecture: new `RagV2AgentService` parallel to `CodingAgentService` using `create_deep_agent` + `subagents=[chunk_analyst]` + `StateBackend`; retrieve-offload-delegate D5 pattern; `rag_search` `@tool` with `backend.upload_files`; Persistence-strategy decision = keep `StateBackend` ephemeral; v1-cutover DEFERRED to post-Testing with G1-G6 proof gate + downstream-consumer audit; 5 test files + G1-G6 closure matrix sharpened to 24 pytest node IDs). Feature health 80% (R:1 A:1 D:1 I:1 T:0). Next: Programming TDD `omt_tdd{op:testlist → red → green → refactor → done}` (two-hats: RED tests/ only, GREEN/REFACTOR src/ only), then Testing (`test_report.md`).
