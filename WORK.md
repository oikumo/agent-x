...
- [x] **feature_kb_akb.application_knowledge_base** — DONE (2026-08-08): UNIFIED concept-altitude index 437 records via `kb_compiler.py build`; g.kb gate + kb_bootstrap inject wired; 21/21 test_kb_* green. Details: `.projects/meta/feature_kb_akb/PROJECT.md` + git log.
- [x] **feature_024.no_tui_full_features** — DONE (2026-08-08): 5 console-parity bug cats resolved; `agent_controller.py` 350→284 LOC via `DemoController` extraction (god-controller gate cleared); 80/80 + 329/329 green. Subtask `feature_024.react_empty_input_reprompt` — empty Enter re-prompts (was quit). Details: .sandbox/feature_024_console_parity_bugs.md + git log.
