...
- [x] **feature_029.rag_v2_slash_commands** — DONE 2026-08-16: rag_v2 console REPL → hybrid slash grammar (bare text = chat; `/help /search /repos /use /create /ingest /status /reset /quit` deterministic, no-LLM). Numeric menu + fake `[3] chat` deleted (`show_chat` cut from ABC + controller). Streaming surfaces tool activity: `_dispatch_stream_delta` routes AI `tool_calls`→`on_tool_call`, tool msgs→`on_tool_result` (≤120 chars); `» search:`/`» analyst:`/`« search:` lines. Rename: `rag_search`→`search_documents`, `rag_ingest_status`→`ingestion_status` (clean cut; 19 feature_027 pins updated). 51 new tests; feature_027 38/38; suite 1335 passed. Test report @ `6.testing/features/feature_029.rag_v2_slash_commands/test_report.md`.
