...
- [x] **META HARNESS DSL** (refactor.meta_harness_dsl; supersedes refactor.meta_harness) — `.sandbox/meta_harness_refactor_plan.md` (anchor `a7163df`; workstreams R0–R8 **ALL DONE**). **DONE: R0 (drift/config/hygiene) · R1 (shared lib omt_shared.ts) · R2 (enforcer split → lib/enforcer/ ×7, single bootstrap) · R3 (tdd_check.py → tdd/ package) · R4 (ledger rotation, omt_done reachable) · R5 (docs single-source: AGENTS.md↔META_HARNESS.md re-synced, test_omt_docs_drift_pins.py ×9) · R6 (think index append-only, compact digest, session.start hook deleted) · R7 (T1 startup pin, T2 WORK.md diet 17.5→10.3 KB, T3 nav-reminder deferral live-verified, T5 budget pins: AGENTS ≤5KiB/WORK ≤14KiB/scratchpad ≤6KiB/nav tip ≤512B/digest ≤1KiB; T4/T8 deferred) · R8 (OMT-HDL-1, plan App D: `.meta/META_HARNESS.omt` v3 source + `scripts/omt/harnessc.py` compiler; IR + nav.index + AGENTS.md + jsonc projections; live-verify 2/2, bun probe 18 tools == IR keys, nav-from-index carries .omt src lines; AGENTS.md generated projection user-ADOPTED 2026-07-26; META_HARNESS.md retired → GENERATED stub; budgets: agents_md 4273/5120, work_md 10431/14336, scratchpad 5851/6144, tool_schemas 1484/2560 — all OK; 226 records 0 errors; suite 1011 + 3 known feature_018).** Per-workstream narratives: git history of this file + plan §0 audit. Plan doc: `.sandbox/meta_harness_refactor_plan.md` (moved 2026-07-26 from `.ws/sandbox/`).
...
- R1 open finding FIXED (R8): runGrep argv += `-H` — explicit-`file` grep keeps the `file:` prefix; the index path filters `r.src===file`.
