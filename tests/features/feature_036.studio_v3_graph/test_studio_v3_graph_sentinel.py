"""Sentinel bridge for feature_036.studio_v3_graph.
...
Vitest — design_001 B11/C10: `omt_tdd` is pytest-shaped; the runner mismatch
was declared in the Programming phase scope (feature_034 A11 precedent) — and
lives at `tools/petri-net-studio/tests/**` (274 tests at feature completion:
the feature_035 base 245 + 2 engine-additive markingFromKey + 7 projection +
8 animation + 4 store-graph + 8 gallery; evidence @
`6.testing/features/feature_036.studio_v3_graph/test_report.md`).
...
VITEST_SUITE_FILES = [
    ...
    "tests/ui/graphProjection.test.ts",
    "tests/ui/animation.test.ts",
    "tests/ui/gallery.test.ts",
    "tests/independence.test.ts",
]

NEW_SRC_FILES = [
    "src/ui/graphProjection.ts",
    "src/ui/animation.ts",
    "src/ui/GraphExplorer.tsx",
    "src/ui/Gallery.tsx",
...
def test_vitest_suite_files_exist() -> None:
    ...
    feature_036 NEW source files, the independence script, the vector
    generator, and the conformance-vector corpus exist where the design pinned
    them (design §3 layout, §7, §8)."""
    ...
    missing_new = [f for f in NEW_SRC_FILES if not (STUDIO / f).is_file()]
    assert not missing_new, f"feature_036 source files missing under {STUDIO}: {missing_new}"
...
def test_vitest_suite_green() -> None:
    """Execute the canonical Vitest suite (274/274 at feature completion).
