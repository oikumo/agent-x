...
**A pure-Python weighted Place/Transition Petri-net library for agentx** — two clearly separated layers: an **execution/model layer** (places, transitions, weighted arcs, markings, enabledness, atomic firing) and an **analysis layer** (reachability, reachability graph, firing sequences, deadlocks, bounds, incidence matrix, P/T-invariants, liveness, SCC, coverability tree) — with explicit, completeness-aware semantics (`complete=True/False`, no silent limits, no overclaiming), following the requirement doc as the starting point.
...
A new, standalone library in `src/agentx/model/petri_net/` (layout per requirement doc §35, adapted to the agentx `src/agentx/model/<pkg>/` convention: `model.py`, `analysis.py`, `coverability.py`, `graph.py`, `simulator.py`, `errors.py`, plus tests under `tests/model/petri_net/`). The library implements:

- **Model layer** — weighted P/T net `N = (P, T, F, W, M0)`; places hold integer token counts, transitions fire atomically when all input requirements are met, arcs carry positive integer weights; core API per doc §9/§10 (`add_place`, `add_transition`, `add_input`, `add_output`, `is_enabled`, `is_enabled_at`, `fire`, `fire_marking` (pure), `enabled_transitions`, `current_marking`, `reset`).
- **Analysis layer** — `PetriNetAnalyzer` operating on the model without mutating its live marking and without UI/database/visualization coupling: `reachable_markings`, `reachability_graph`, `firing_sequence_to`, `deadlocks`, `bounds`, `incidence_matrix`, `place_invariants`, `transition_invariants`, `transition_liveness`, `is_live`, `strongly_connected_components`, `coverability_tree`.
- **Completeness semantics** — result objects expose `value: bool | None`, `complete: bool`, `explored_states`, `reason`; every exploration API takes `max_states` / `max_depth` / `time_limit` and never claims proof from a truncated search (§27–28, §39).
...
- **Not a simulation-as-analysis shortcut.** The `simulator.py` module (one-path policies, doc §26) is explicitly NOT a substitute for exhaustive analysis.
...
The full requirement is `.meta/doc/petri_nets/petri_net_python_coding_agents.md` (41 sections). Non-negotiable requirements this project inherits:
...
4. Edge-case coverage: self-loops, no-input (always enabled), no-output transitions, parallel transitions (distinct labels), zero-token places, empty-net decision, duplicate-arc rule (§8, §38).
5. Exact integer/rational arithmetic for incidence matrix and invariants (§18–19) — implies a dependency decision (sympy recommended by doc; **not currently in `pyproject.toml`** — open decision, see D4).
6. Completeness-explicit results and analysis limits (§27–28, §39): `True` = proven, `False` = disproven, `None` = unknown/incomplete.
7. Coverability (Karp–Miller-style) as a separate module; truncated BFS is never labeled "unbounded" (§17).
8. Definition of Done = doc §40 items 1–19; minimum analysis toolkit = doc §36 checklist.
...
- **(b) A trustworthy analysis toolkit.** The §36 minimum set — reachability, reachability graph, shortest firing sequence, deadlocks, bounds, incidence matrix, P/T-invariants (exact arithmetic), transition liveness on complete finite graphs, SCC, coverability tree — each returning completeness-explicit results. Verified by DoD items 7–19 tests, including "unknown" cases.
...
**What is NOT the vision:** not a UI feature; not a research-grade state-space optimizer; not a graphviz/reporting tool; not a simulation engine with analysis sprinkled on top.
...
1. **Model layer** — `PetriNet` with the doc §9 API: places (initial tokens), transitions, weighted input/output arcs, enabledness (at-marking and current), atomic firing, pure `fire_marking`, `reset`, canonical tuple markings (`place_order`/`place_index`), structural queries (`pre_set`/`post_set`, §24).
2. **Analysis layer (minimum toolkit, doc §36)** — `reachable_markings` (BFS, predecessor map), `reachability_graph`, `firing_sequence_to`, `deadlocks`, `bounds`, `incidence_matrix` (exact), `place_invariants`, `transition_invariants` (exact arithmetic), `transition_liveness`/`is_live` (finite complete graphs, reverse-BFS), `strongly_connected_components` (Tarjan/Kosaraju in a generic `graph.py`), `coverability_tree` (separate `coverability.py`).
3. **Completeness semantics + limits** — result dataclasses with `complete`/`reason`; `max_states`/`max_depth`/`time_limit` on every exploration API; no hidden hard-coded limits.
4. **Edge cases** — self-loops, no-input/no-output transitions, parallel transitions, zero-token places, empty net (allowed; consistent analysis), duplicate-arc policy (documented).
5. **Tests** — `tests/model/petri_net/` (currently a placeholder stub): model, analysis, coverability, graph; both positive results AND "unknown" cases (doc §30, §40-19).

### Out of scope (explicit non-goals, draft)
...
- **No siphons/traps in the first iteration** (§25 — advanced optional; minimal-siphon/trap enumeration only for tiny nets if at all).
- **No home-marking analysis in the first iteration** (§22 — advanced optional; `is_home_marking` may come later).
...
- **No simulation policies beyond the minimal `simulator.py`** one-path helper.

### Success criteria (draft — mapped to requirement doc)

- Doc §40 DoD items 1–19 all demonstrably pass (execution items 1–6; analysis items 7–19).
- Doc §36 minimum analysis toolkit checklist complete.
...
- **What changes (will change):** new `src/agentx/model/petri_net/` package + real tests under `tests/model/petri_net/` (replacing the placeholder `test_petri_net.py` stub); `pyproject.toml` gains the invariant-arithmetic dependency chosen in D4 (likely `sympy`).
...
- **What is deferred (future, not this project):** feature_001 integration, siphons/traps, home markings, DOT/JSON export, state-space optimization.
...
- **D2 — Library location `src/agentx/model/petri_net/`** (agentx model-layer convention, e.g. `session/`, `rag_v2/`), module layout per doc §35 (`model.py`, `analysis.py`, `coverability.py`, `graph.py`, `simulator.py`, `errors.py`, `__init__.py` exposing `PetriNet`, `PetriNetAnalyzer`).
...
- **D4 — Exact arithmetic for invariants: sympy (doc-recommended) vs pure-Python.** Doc §18 recommends `sympy` for null-space over exact rationals. `pyproject.toml` currently has `numpy` but **no sympy**. Decision at approval time: (a) add `sympy` dependency (doc-recommended, lowest risk), or (b) implement a small exact-rational nullspace in pure Python (zero deps, more code). Draft lean: (a).
...
- **D7 — Edge-case policy** — self-loops legal; no-input transitions always enabled; no-output transitions legal (consume-and-vanish); parallel transitions distinct by name; zero tokens meaningful; empty net allowed with consistent analysis (`()` initial marking, no enabled transitions); duplicate arcs: reject (doc §38 "simplest API should reject duplicates").
- **D8 — TDD is mandatory when implemented** — `major_feature` → feature_016 auto-activates at Programming; close via `omt_tdd{op:done}`, not `omt_skip`.
- **D9 — feature_001 relationship: consumer, not scope** — this library is the generic substrate; `feature_001.session_user_objectives_driven_by_Petri_Net` (internal state, USER_OBJECTIVES.md) is a separate future feature that consumes it.
...
<!-- TA: xref: xref: tests/model/petri_net/test_petri_net.py is a June placeholder stub (assertTrue(True)); src/agentx/model/petri_net/ does not exist; feature_001.session_user_objectives_driven_by_Petri_Net (FEATURE.md: internal_state module, USER_OBJECTIVES.md CRC update) is the FUTURE CONSUMER, not part of this project's scope (D9). Verified 2026-08-16 at iter-0. -->
<!-- TA: gotcha: gotcha: doc §18 recommends sympy for exact invariant nullspace, but pyproject.toml has numpy only — the D4 decision (add sympy vs pure-Python exact-rational nullspace) is OPEN at approval time. Draft lean is sympy (doc-recommended, lowest risk). Do not silently implement invariants with numpy floating-point nullspace — doc §18 explicitly rejects float rank/null-space for Petri-net invariants. -->

- **Requirement doc (the anchor)** — `.meta/doc/petri_nets/petri_net_python_coding_agents.md`. 41 sections covering foundations (§1–2), model layer (§3–10), analysis layer (§11–25), design rules (§26–34), structure (§35), minimum feature set (§36), end-to-end example (§37), edge cases (§38), no-overclaiming (§39), Definition of Done (§40), final mental model (§41).
