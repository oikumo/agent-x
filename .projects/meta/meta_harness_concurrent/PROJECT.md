...
> Status: **draft** · **v0.4 (2026-08-30)** — created by `project.py new` (v0.1 scaffold); project definition written on resume, scope directive D1 (meta harness only, not agentx) locked same session. Iterate freely (non-gated); spawn features with `new_feature.py "<name>" --type <tt> --project meta_harness_concurrent`; log sessions in CURRENT_STATE.md (newest on top).
> **Architecture decisions locked via IDEA-001 (file-backed net control) + IDEA-002 v3 (compositional net-of-nets):** the net is an **additive observability/guidance layer** (not a control plane); 3-feature core + 2 optional phase-2; `petri-net-json` v1 + sidecar for live marking; single `omt_net` tool with ops.

---

## New Session Quick Start

> One line: A **meta-harness-scoped** design project — an **additive Petri-net net-of-nets observability/guidance layer** over the existing phase-FSM + ledger control plane (D3), modeling concurrent projects/features and resource capacities as a flat supervisor net + per-project/per-feature subnets with boundary ports, giving the agent structured conflict/deadlock visibility and analyzer-backed blocking (not gate override). **Meta harness only — NOT agentx (D1).**

**Next:** approve the **3-feature core roadmap** (feature_039.adaptive_net_engine, feature_040.net_composition_supervisor, feature_041.resource_places_concurrency — feature_042/043 are optional phase-2), then scaffold: `uv run scripts/omt/new_feature.py "adaptive net engine" --type minor_feature --project meta_harness_concurrent`.

---

## Summary (one line)

**An additive Petri-net net-of-nets observability/guidance layer for the META HARNESS** — a flat supervisor net composing per-project/per-feature subnets through boundary ports (`feature_ready`, `resource_token`, `goal_satisfied`) with resource places (`agent_attention`=1, `src_edit_capacity`, `tests_capacity`, `harness_surface_round`, `e2e_receipt`) making concurrency conflicts structurally visible; the analyzer blocks invalid fires, drift checks reconcile net vs ledger at every gate; single `omt_net` tool (ops: probe/fire/splice/synthesize/invariant); `petri-net-json` v1 + sidecar for live marking. Scoped to harness tooling only (D1) — agentx stays out.
...
- An **additive observability/guidance layer** (IDEA-003) over the existing phase-FSM + ledger control plane: the net **mirrors, guides, and can block** via the analyzer, but **never replaces the gates** (D3, IDEA-003.D1/D2). The ledger + phase FSM remain the primary approval authority.
- A **flat supervisor net** composing per-project/per-feature subnets through boundary ports (`feature_ready`, `resource_token`, `goal_satisfied`) — composition is a modeling overlay (flat union + collision-safe `f{N}_` prefixes + boundary places), not engine hierarchy.
...
- **Single `omt_net` tool with ops** (probe/fire/splice/synthesize/invariant) — follows `omt_q` precedent, avoids harness surface churn (F5).
- **Net↔reality sync** via `omt_net_sync` op: bootstrap + re-sync on `project.py` lifecycle events + `new_feature.py --project` link; sync = deterministic proposal through splice path, never silent (D4).
...
- **NOT a redesign of the OMT gate/phase FSM.** Existing before/after gates, phase FSM, TDD pipeline, and `omt_*` tool contract stay. The net is an *additive observability/guidance layer* — it mirrors, guides, and blocks via the analyzer; it does not replace the enforcement layer (D3).
...
- **NOT a control plane.** The net is an observability/guidance layer; gates own approval.

---

## Scope & success criteria

**Scope (declared, not executed; design basis = `.sandbox/pause_2026-08-30.md` + the candidate idea docs under `ideas/` — IDEA-001 file-backed control, IDEA-002 compositional architecture, IDEA-003 additive observability layer. Roadmap numbers verified available: highest done = feature_038, next slots 039+.)**
...
**Core = features 1–3.** Features 4–5 are phase-2, only if core proves valuable (IDEA-003 §6 re-scope, not cancellation).
...
- The harness net mirrors actual project/feature state (WORK.md + `.projects/` + feature dirs) via `omt_net_sync` bootstrap + lifecycle hooks, and detects structural conflicts/deadlocks mechanically — demonstrable on a ≥2-concurrent-feature scenario.
...
- **D7 — Net-vs-ledger reconciliation as drift check (IDEA-001 item 2 resolved):** `omt_net{op:invariant}` runs drift check at **every `omt_complete` exit**; ledger = primary approval authority; net blocks fires via analyzer; logs to `harness.net.drift.jsonl` (mirrors `omt_q{op:drift}` envelope).
...
- **D9 — 3-feature core roadmap (IDEA-001 item 4 resolved):** feature_039 (engine), feature_040 (composition), feature_041 (resources) = core; feature_042 (synthesis), feature_043 (dashboard) = optional phase-2.
- **D10 — Single `omt_net` tool with ops (IDEA-002.D3):** one tool, ops = probe/fire/splice/synthesize/invariant; follows `omt_q` precedent; avoids F5 harness surface churn.
...
- `IDEA-002` — `.projects/meta/meta_harness_concurrent/ideas/idea-002-compositional-net-of-nets-architecture.md` (v3) — compositional net-of-nets architecture, additive layer, sidecar/overlay schemas, structural transactions, tool surface, sync hooks, all open items resolved in design.
