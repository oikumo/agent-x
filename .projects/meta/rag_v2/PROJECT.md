...
<!-- TA: xref: ANALYSIS-PARTIAL (2026-08-09) deepagents stack verified from feature_025 ship + library source (deepagents/graph.py:268-893, backends/state.py, middleware/subagents.py). create_deep_agent(...) at coding_agent_service.py:159-172 passes model/tools/system_prompt/backend|StateBackend()/checkpointer|InMemorySaver/memory|default["./AGENTS.md"]/skills|default["./src/agentx/model/coding/coding_skills/"]/middleware=[create_summarization_tool_middleware(llm,backend)]. The 4 middleware PROJECT.md Vision names (Filesystem/Summarization/Memory/Skills) are AUTO-ASSEMBLED by create_deep_agent (graph.py:816-870) based on memory/skills/subagents/permissions kwargs — NOT visible in coding_agent_service.py body. Order verified: Skills->Filesystem->SubAgents->Summarization->Patch->[user middleware splices here]->ToolExclusion->PromptCaching->Memory. subagents= kwarg ABSENT from the agentx call, but create_deep_agent AUTO-ADDS a default general-purpose subagent (graph.py:750-814) so the task() tool + SubAgentMiddleware are already live. v2's retrieval @tool pattern = from langchain.tools import tool; dataclass return type; Google-style docstring=description; thin wrapper over _*_impl. Offload of >20k-token results performed by FilesystemMiddleware (auto-wired) — v2 MAY NOT need explicit backend.upload_files() for large results, depending on whether it wants deterministic subagent read_file/grep access. StateBackend is EPHEMERAL/thread-scoped — chunks uploaded this turn do NOT survive reset_conversation(); if v2 needs persistent chunks, swap to FilesystemBackend/StoreBackend/CompositeBackend. v2 ADDING subagents=[chunk-analyst] would coexist with the auto-added general-purpose default UNLESS v2's profile disables it (GeneralPurposeSubagentProfile(enabled=False)). -->
<!-- TA: gotcha: ANALYSIS-PARTIAL (2026-08-09) G1-G3 STALE: the v1-gap->v2 closure matrix at lines 178-187 lists G1/G2/G3 as constraints v2 owns, but v1 ALREADY SHIPS them in the working tree. Verified: G1 rag_create_repository_controller.py:96-126 _create_repository fully implemented (137 lines, mkdir+RagDatabase.create_if_not_exists); G2 rag_repository_selection_controller.py:57-76 returns cached RagRepository with 1-based->0-based index map + bounds check (NOT None); G3 rag_controller.py:66-107 get_rag_state returns populated RagState (not commented, consumes Rag.database_exists/documents_exist/get_ingested_url). The 3 feature_002 design docs (design_001-003) describe PRE-implementation state; commit cdeb15f shipped both the designs AND the code same day (2026-06-21); FEATURE.md §Status block never refreshed. v2 TRUE scope per current code = G4 (PDF/MD ingestion CONFIRMED, only web via Tavily) + G5 (multi-repo session switch CONFIRMED, only local current_rag_repository field, no switch command propagates to peer agents) + G6(b) (deepagents CONFIRMED, zero refs in RAG tree) + NARROW G6(a) (3 RAG inner views RagChatView/RagRepositorySelectionView/RagWebIngestionView are bare classes w/o I<X>View ABC pair; ConsoleProvider has no create_*_view factories for them — but the OUTER integration via IRagView/IRagViewPartner/RagShowCommand IS parity). When writing analysis_001_*.md: cap G1-G3 scope as "v1 already ships; v2 mirrors for parity, not for closure" — do NOT propose reimplementing them as net-new. -->
